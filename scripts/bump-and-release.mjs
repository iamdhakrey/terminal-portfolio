...
class Spinner {
    ...
    start() {
        ...
        this.timer = setInterval(() => {
            const frame = paint(C.cyan, this.frames[this.index++ % this.frames.length]);
            this.stream.write(`\r${frame} ${this.text}`);
        }, 80);
        return this;
    }

    succeed(msg) { this._stop(paint(C.bGreen, '✔'), msg || this.text); }
    fail(msg) { this._stop(paint(C.bRed, '✖'), msg || this.text); }
    warn(msg) { this._stop(paint(C.bYellow, '⚠'), msg || this.text); }
    info(msg) { this._stop(paint(C.bBlue, 'ℹ'), msg || this.text); }
...
class Logger {
    ...
    info(msg) { return this._record('info', 'ℹ', C.bBlue, msg); }
    ok(msg) { return this._record('ok', '✔', C.bGreen, msg); }
    warn(msg) { return this._record('warn', '⚠', C.bYellow, msg); }
    error(msg) { return this._record('error', '✖', C.bRed, msg); }
    step(msg) { return this._record('step', '→', C.bMagenta, msg); }
    debug(msg) { return this._record('debug', '·', C.gray, msg); }
    ...
    summary() {
        const counts = {};
        for (const { level } of this.logs) counts[level] = (counts[level] || 0) + 1;
        const lines = [
            '',
    ...
    dump() {
        ...
        for (const { ts, level, message } of this.logs) {
            console.log(`${dim(ts)} ${paint(C.gray, level.padEnd(6))} ${message}`);
        }
        console.log(paint(C.gray, '────────────────────────────────────────\n'));
    }
}
...
function parseCommits(rawLines) {
    const groups = { breaking: [], feat: [], fix: [], perf: [], refactor: [], docs: [], chore: [], other: [] };
    ...
    for (const line of rawLines) {
        ...
        else if (/^chore[:(]|^build[:(]|^ci[:(]/.test(msg)) groups.chore.push({ hash, msg });
...
function renderChangelog(version, commits, repoUrl = '') {
    ...
    const totalCommits = Object.values(groups).reduce((s, a) => s + a.length, 0);
    ...
    const badgeParts = [`🗓 ${date}`, `📦 ${totalCommits} commit${totalCommits !== 1 ? 's' : ''}`];
...
class ReleaseManager {
    ...
    _repoUrl() {
        try {
            ...
            return remote.replace(/\.git$/, '').replace(/^git@([^:]+):/, 'https://$1/');
        } catch {
            return '';
        }
    }
    ...
    getNextVersion(current, type) {
        ...
        switch (type) {
            ...
            case 'premajor': return `${major + 1}.0.0-alpha.1`;
            case 'preminor': return `${major}.${minor + 1}.0-alpha.1`;
            case 'prepatch': return `${major}.${minor}.${patch + 1}-alpha.1`;
            case 'prerelease': {
                if (pre) {
    ...
    updatePackageVersion(newVersion) {
        const spin = new Spinner(`Updating package.json → ${newVersion}`).start();
        const pkg = this._pkg();
    ...
    getCommitsSinceLastTag() {
        try {
            ...
            const raw = execSync(`git log ${lastTag}..HEAD --format="%H %s"`, { encoding: 'utf8', stdio: 'pipe' }).trim();
            return raw ? raw.split('\n') : [];
        } catch {
            this.log.warn('No previous tags found — using full commit history');
            const raw = execSync('git log --format="%H %s"', { encoding: 'utf8', stdio: 'pipe' }).trim();
            return raw ? raw.split('\n') : [];
        }
    }
    ...
    validateGitState() {
        ...
        try {
            ...
            const dirty = status
                ...
                .filter(l => !l.includes('package.json') && !l.includes('CHANGELOG.md'));

            if (dirty.length > 0) {
                spin.warn('Working tree has uncommitted changes (non-blocking)');
    ...
    createGitTag(version) {
        ...
        this._exec(`git commit -m "chore(release): v${version} 🚀"`, `Committing release v${version}`);
        this._exec(`git tag -a v${version} -m "Release v${version}"`, `Tagging v${version}`);
    ...
    createGitHubRelease(version, commits) {
        ...
        try {
            ...
            try { execSync(`rm "${notesFile}"`); } catch { /* ignore */ }
    ...
    _printBanner(current, next, type) {
        ...
        const row = (content) => `${paint(C.bCyan, '║')} ${pad(content, width - 3)} ${paint(C.bCyan, '║')}`;
        ...
        console.log(`${paint(C.bCyan, '║')} ${padAnsi(
    ...
    _printSuccess(version, elapsed) {
        ...
        console.log(`\n${paint(C.bGreen, '╔══════════════════════════════════════════════════╗')}`);
        console.log(`${paint(C.bGreen, '║')}  ${bold('✅  Release completed successfully!')}               ${paint(C.bGreen, '║')}`);
        console.log(`${paint(C.bGreen, '║')}  ${dim(`Version  `)}${paint(C.bWhite, `v${version}`).padEnd(10)}  ${dim(`Time  `)}${paint(C.bWhite, `${secs}s`)}           ${paint(C.bGreen, '║')}`);
        console.log(`${paint(C.bGreen, '╚══════════════════════════════════════════════════╝')}\n`);
    }

    // ── Main entrypoint ──────────────────────────────────────────────────────

    async release(releaseType, options = {}) {
        const { dryRun = false, skipGitHub = false, skipPush = false } = options;
        ...
        // Gather commits
        const spin = new Spinner('Collecting commits since last release').start();
        const commits = this.getCommitsSinceLastTag();
